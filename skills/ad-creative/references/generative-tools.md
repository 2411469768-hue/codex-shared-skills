...
**Prompting rules for Seedance 2.0 ad prompts:**
- Assign every reference a job. Use images for stable product/character/brand/first-frame/last-frame/scene cues, videos for action/camera/rhythm/transitions/effects, and audio for BGM, voice tone, dialogue style, rhythm, or sound effects.
- Avoid vague references like "use @Video1". Write the exact role, such as "reference @Video1's camera push-in and cut rhythm; keep the product appearance from @Image1."
- For 8-15 second ads, write timed beats: 0-3s hook, 3-6s product/action reveal, 6-10s benefit or usage moment, 10-15s final hero shot or CTA space. Keep 4-5 second ads to one scene and one primary motion.
- Check limits before planning: up to 9 images, 3 videos, 3 audio files, 12 files total; reference video/audio should be 2-15 seconds.
- Avoid conflicting directions in one beat, such as static camera plus orbit, close-up plus full-body, or multiple scene changes in a short clip.
- Keep text overlays, prices, captions, and CTA copy out of the generated image when legibility matters; add them in editing or Remotion unless the model/tool is specifically chosen for text rendering.
