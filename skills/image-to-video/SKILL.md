...
- **Five-dimension scan before execution** — identify what images, videos, and audio already lock, then only fill blank dimensions in the prompt. Do not spend Seedance's short prompt budget restating rich references.
- **Compress preservation clauses** — use one compact asset-lock sentence for identity/product/costume stability, and move repeated "do not change" failures into the negative/avoidance wording when the model supports it or the prompt tail when it does not.
...
- **Assign each ref a role** — do not write vague references. Say whether image 1 is the subject, outfit, product, first frame, last frame, or scene; whether video 1 controls action, camera movement, rhythm, transitions, effects, facial expression, or ambient sound; whether audio 1 controls BGM rhythm, voice tone, dialogue style, or sound effects.
- **Use script-driven time segments for longer clips** — for 8s+ outputs, split by action beats and difficulty instead of fixed `0-3s`, `3-6s`, `6-10s` blocks. Keep one primary action and one primary camera goal per segment.
- **Avoid instruction conflicts** — don't combine static camera with orbit/follow camera in the same segment, or ask for a close-up and full-body framing at once.
- **Reference media specs** — videos / audio must be 2–15s; audio < 15MB.
- **Media count check** — Seedance supports up to 9 images, 3 videos, 3 audio files, 12 files total. If the user gives more, select the few assets that control identity, product/wardrobe, scene, motion, and audio rhythm.
