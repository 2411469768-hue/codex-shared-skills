...
This is a menu, not a mandatory long template. Omit or compress any part already locked by uploaded assets; spend text only on blank dimensions and the desired change.

### Five-Dimension Scan Before Writing
Before drafting, scan the script and assets across five dimensions:

1. Absolute subject and physical motion: character/product, posture, action, placement.
2. Environment field and emotional lighting: scene, light direction, color tone, spatial anchors.
3. Optics and camera scheduling: shot size, lens feel, push/pull, pan/tilt, orbit, tracking.
4. Timeline and state evolution: how the action moves from starting state to result state.
5. Aesthetic medium and base rendering: phone realism, film look, oil-painting look, material texture, render style.

Mark which dimensions are already locked by uploaded assets and which are blank. Images usually lock identity, product/character appearance, first frame, background, and some style. Videos usually lock action, camera path, rhythm, transitions, and expression changes. Audio usually locks BGM, beat, voice tone, ambience, or effects. Do not rewrite locked dimensions; only fill missing dimensions so text features do not compete with image/video features for attention.

Run an attention-pollution check: bind colors, materials, lighting, props, and style words to the exact object they belong to. Convert vague style words into visible evidence such as light direction, focal distance, framing, material, color, spatial anchors, and action.

### Prompt Compression Rule
If a user provides a bloated prompt or rich references, rewrite it into a compact execution structure:

1. Asset lock: one short block for identity/product/costume/scene details that must remain stable.
2. Blank-dimension instructions: only the missing action, camera, timeline, audio, or style evidence.
3. Script-driven timeline: time ranges based on action beats, not fixed chunks.
4. Negative prompt: only hard failures, such as identity drift, deformed hands, background jumps, text, watermarks, and motion stutter.

Do not repeat the same preservation constraint in both positive and negative form. Put repeated "do not change" clauses in the negative prompt unless a specific failure needs emphasis.

### Script-Driven Timelines (Recommended for 8s+ or Complex Motion)
For precise control, break the prompt into timed segments based on the script or reference action, not fixed 0-3/3-6/6-9 blocks:

```text
[time range from script logic]: [subject placement, action, gaze/head, hands, camera, state change, ending pose]
```

First identify the starting state, trigger action, main change, and result state. For narrative clips, identify character intent, action progress, emotional change, and result. Then allocate time according to total duration and action difficulty: simple beats can be short; walking, turning, transformations, effects growth, object interactions, or fabric/product motion need longer transitions.

Keep one primary action and one primary camera goal per segment. When the subject performs a large movement, keep camera motion restrained; when the camera moves strongly, simplify the subject's action. Avoid packing scene changes, explosions, major turns, close-up cuts, and camera pushes into the same beat.
...
4. **Scan and structure the prompt**:
   - Mark which of the five dimensions are locked by assets and which are blank
   - Open with subject and scene setup only when assets do not already lock them
   - Add script-driven action beats for videos > 8s or complex motion
   - Specify camera movements
   - Add audio/sound design
   - Include style modifiers only when grounded in visible evidence
5. **Check constraints**: Verify total files ≤ 12, no real human faces, durations within limits
6. **Optimize**: Remove ambiguity and duplicate preservation clauses, prevent attention pollution, ensure each @reference has a clear role
...
8. **Mechanical time blocks**: Don't force 0-3/3-6/6-9 segments when the script has different action beats
9. **Over-describing locked assets**: Don't restate subject, scene, light, or style already locked by an image/video; fill only missing dimensions
10. **Style-word pollution**: Don't rely on vague terms like cinematic, premium, emotional, or epic unless grounded in visible lighting, lens, color, material, space, or action
...
showcase video.
Opening beat: Product enters frame with dynamic rotation, close-up on surface
texture and logo details; keep camera movement simple while the product rotates.
Feature beat: Multiple angle transitions — front, side, back — with product
highlight scanning light effects.
Usage beat: Product in lifestyle context showing usage scenario.
Resolution beat: Hero shot holds on the product while background music builds
to resolution. Add any brand text in post-production, not as generated scene text.
