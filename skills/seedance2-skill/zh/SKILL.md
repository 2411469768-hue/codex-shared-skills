...
这是可选菜单，不是必须全部写满的万能长模板。上传素材已经锁定的部分要省略或压缩，只把文字留给空白维度和需要变化的内容。

### 撰写前五维扫描
正式写提示词前，先扫描剧本和素材的五个维度：

1. 绝对主体与物理动势：人物/产品、姿态、动作、站位。
2. 环境场与情绪光影：场景、光线方向、色调、空间锚点。
3. 光学与摄影机调度：景别、焦距感、推拉摇移、环绕、跟拍。
4. 时间轴与状态演变：动作如何从起始状态过渡到结果状态。
5. 美学介质与底层渲染：真实手机感、电影感、油画感、材质质感、渲染风格。

标出哪些维度已经被上传素材锁定，哪些还是空白。图片通常锁人物/产品外观、首帧、背景和部分风格；视频通常锁动作、镜头路径、节奏、转场和表情变化；音频通常锁 BGM、卡点、音色、环境声或音效。素材已经锁定的维度不要重复铺写，只补空白维度，避免文字特征和图片/视频特征抢注意力。

输出前做注意力污染检查：颜色、材质、光效、道具和风格词必须绑定到正确对象。把“电影感、高级感、史诗感、氛围感”等抽象词转成可见证据，例如光线方向、焦距感、景别、材质、色彩、空间锚点和动作。

### 提示词压缩规则
当用户提供臃肿提示词或素材信息很满时，重写成克制执行结构：

1. 素材锁定：用一个短块锁定身份、产品、服装、场景等必须稳定的信息。
2. 空白维度指令：只补动作、镜头、时间轴、音频或风格证据中缺失的部分。
3. 剧本驱动时间轴：按动作节拍分配时间，不机械套固定时间块。
4. 负面提示词：只列身份漂移、手部畸形、背景跳变、文字水印、动作卡顿等硬错误。

不要把同一个保持约束同时用正向和反向重复写满。反复出现的“不改变、不漂移、不变形”默认放进负面提示词，除非某个失败点需要单独强调。

### 剧本驱动时间轴（8秒以上或复杂动作推荐使用）
精确控制画面时，按剧本或参考动作拆时间段，不要机械套固定的 0-3/3-6/6-9 秒：

```text
[根据剧本逻辑分配的时间段]：[主体站位、动作、头部/眼神、手部、镜头、状态变化、结束姿态]
```

先拆出起始状态、触发动作、主要变化、结果状态；剧情短片还要拆人物意图、动作推进、情绪变化、结果。再按总时长和动作难度分配时间：简单节拍可以短，走动、转身、变身、特效生长、物体交互、服装/产品动态需要更长过渡。

每段只放一个主要动作和一个主要镜头目标。人物大动作时镜头要稳；镜头大运动时人物动作要简单。避免在同一段里同时换景、爆炸、大转身、切近景和推镜头。
...
8. **机械时间段**：剧本节拍不同就不要强行套 0-3/3-6/6-9 秒
9. **重复描述已锁定素材**：图片/视频已锁定主体、场景、光线或风格时，只补空白维度
10. **风格词污染**：不要只写电影感、高级感、史诗感、氛围感，必须落到光线、焦距、景别、材质、色彩、空间或动作
...
开场节拍：产品以动感旋转入场，特写表面材质和logo细节；产品旋转时镜头保持简单稳定。
卖点节拍：多角度转场展示——正面、侧面、背面——配合产品扫光效果。
使用节拍：产品进入使用场景，展示实际应用。
收束节拍：产品主视觉定格，背景音乐推向高潮；品牌广告语放到后期，不要求模型生成画面文字。
...
4. **扫描并构建提示词**：
   - 标出五个维度里哪些已被素材锁定，哪些仍是空白
   - 只有素材没有锁定主体和场景时，才在开头补主体和场景设定
   - 超过8秒或动作复杂的视频使用剧本驱动时间轴
   - 明确运镜语言
   - 加入音频/音效设计
   - 风格修饰词必须落到可见画面证据
5. **检查约束**：确认总文件数 ≤ 12、无写实人脸、时长在限制范围内
6. **优化打磨**：消除歧义和重复保持约束，避免注意力污染，确保每个 @ 引用都有明确的作用说明
